<template>
  ...
  <p>{{count_c_2}}</p>
  <button @click="count_c_2++">Composition API 2st 카운트 증가</button>
  <p>상태: {{state}}</p>
...
export default {
  ...
  setup() {
    ...
    watch(
      ...
      }, {immediate: true,} // immediate: true 초기값의 변화(undefined -> 0) 파악함
    )
    ...
    const stop = watchEffect(
      ...
      }, {flush: 'post',} //pre: dom이 업데이트 하기 전에 콜백함수를 호출
    )                     //post: dom이 업데이트 하기 전에 콜백함수를 호출
