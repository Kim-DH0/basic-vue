<template>
  ...
  태생적으로 ctrl, alt, shift는 다른 키와 함께 동작하기 때문에 단독으로는 동작을 하지 않는다.
  하지만 다른 키와 같이 누를 경우 exact를 붙이든 안 붙이든 함수는 동작한다.
  ...
  하지만, 다른 키에 안 붙일 경우는 두 군데 붙은 함수가 두 번 동작하게 된다.
  둘 다 안 붙을 수는 없다. 왜냐면 키는 한 번 누를 때마다 동작하는데 동시 누르면 동작 안 한다.
...
<style>
    /* img, input {
    -webkit-animation: vibrate-3 0.1s linear infinite both;
